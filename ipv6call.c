...
#include <termios.h>
#include "l2tp.h"
#include "ipsecmast.h" 	//TODO: need to write in a better way

struct buffer6 *new_payload6 (struct sockaddr_in6 peer)
{
...
inline void recycle_payload6 (struct buffer6 *buf, struct sockaddr_in6 peer)
{
...
void add_payload_hdr6 (struct tunnel6 *t, struct call6 *c, struct buffer6 *buf)
{
...
int read_packet6 (struct buffer6 *buf, int fd, int convert)
{
    unsigned char c;
...
void call_close6 (struct call6 *c)
{
    struct buffer6 *buf;

    struct schedule_entry *se, *ose;
    struct call6 *tmp, *tmp2;
    if (!c || !c->container)
    {
    ...
    if (c == c->container->self)
    {
        ...
        while (se)
        {
            if ((((struct buffer6 *) se->data)->tunnel == c->container)
                || ((struct tunnel6 *) se->data == c->container))
            {
#ifdef DEBUG_CLOSE
                ...
                {
                    ose->next = se->next;
                    if ((struct tunnel6 *) se->data != c->container)
                        toss6 ((struct buffer6 *) (se->data));
                    free (se);
                    se = ose->next;
                }
                else
        ...
        buf = new_outgoing6 (c->container);
        add_message_type_avp6 (buf, StopCCN);
        if (c->container->hbit)
        {
            mk_challenge (c->container->chal_them.vector, VECTOR_SIZE);
            add_randvect_avp6 (buf, c->container->chal_them.vector,
                              VECTOR_SIZE);
        }
        add_tunnelid_avp6 (buf, c->container->ourtid);
        if (c->result < 0)
            c->result = RESULT_CLEAR;
        if (c->error < 0)
            c->error = 0;
        add_result_code_avp6 (buf, c->result, c->error, c->errormsg,
                             strlen (c->errormsg));
        add_control_hdr6 (c->container, c, buf);
...
/*		if (c->dethrottle) deschedule(c->dethrottle); */
        ...
        add_result_code_avp6 (buf, c->result, c->error, c->errormsg,
                             strlen (c->errormsg));
#ifdef TEST_HIDDEN
        add_callid_avp6 (buf, c->ourcid, c->container);
#else
...
void destroy_call6 (struct call6 *c)
{
    /*
...
#ifdef IP_ALLOCATION1	//RY: disabled
    if (c->addr)
        unreserve_addr6 (c->addr);
#endif
	...
	*/
    ...
    if (c->lac)
    {
            ...
            c->lac->active)
        {
            ...
            tv.tv_usec = 0;
            c->lac->rsched = schedule (tv, magic_lac_dial6, c->lac);//RY: made it to call ipv6 version
        }
    }
...
struct call6 *new_call6 (struct tunnel6 *parent)
{
    unsigned char entropy_buf[2] = "\0";
    struct call6 *tmp = malloc (sizeof (struct call6));
...
/*	tmp->rws = -1; */
    ...
    tmp->lns = parent->lns;
    tmp->lac = parent->lac;
    //tmp->addr = 0;//RY: updated for IPv6 address
...
struct call6 *get_tunnel6 (int tunnel,  int port)
{
    struct tunnel6 *st;
    if (tunnel)
    {
...
struct call6 *get_call6 (int tunnel, int call, struct in6_addr addr, int port,
