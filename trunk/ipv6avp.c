...
#define AVP_MAX 39
//RY: start
extern char *cdn_result_codes[];
extern char *stopccn_result_codes[];
//RY: end
struct avp6 avps6[] = {

    {0, 1, &message_type_avp6, "Message Type"},
    {1, 1, &result_code_avp6, "Result Code"},
    {2, 1, &protocol_version_avp6, "Protocol Version"},
    {3, 1, &framing_caps_avp6, "Framing Capabilities"},
    {4, 1, &bearer_caps_avp6, "Bearer Capabilities"},
    {5, 0, NULL, "Tie Breaker"},
    {6, 0, &firmware_rev_avp6, "Firmware Revision"},
    {7, 0, &hostname_avp6, "Host Name"},
    {8, 1, &vendor_avp6, "Vendor Name"},
    {9, 1, &assigned_tunnel_avp6, "Assigned Tunnel ID"},
    {10, 1, &receive_window_size_avp6, "Receive Window Size"},
    {11, 1, &challenge_avp6, "Challenge"},
    {12, 0, NULL, "Q.931 Cause Code"},
    {13, 1, &chalresp_avp6, "Challenge Response"},
    {14, 1, &assigned_call_avp6, "Assigned Call ID"},
    {15, 1, &call_serno_avp6, "Call Serial Number"},
    {16, 1, NULL, "Minimum BPS"},
    {17, 1, NULL, "Maximum BPS"},
    {18, 1, &bearer_type_avp6, "Bearer Type"},
    {19, 1, &frame_type_avp6, "Framing Type"},
    {20, 1, &packet_delay_avp6, "Packet Processing Delay"},
    {21, 1, &dialed_number_avp6, "Dialed Number"},
    {22, 1, &dialing_number_avp6, "Dialing Number"},
    {23, 1, &sub_address_avp6, "Sub-Address"},
    {24, 1, &tx_speed_avp6, "Transmit Connect Speed"},
    {25, 1, &call_physchan_avp6, "Physical channel ID"},
    {26, 0, NULL, "Initial Received LCP Confreq"},
    {27, 0, NULL, "Last Sent LCP Confreq"},
    {28, 0, NULL, "Last Received LCP Confreq"},
    {29, 1, &ignore_avp6, "Proxy Authen Type"},
    {30, 0, &ignore_avp6, "Proxy Authen Name"},
    {31, 0, &ignore_avp6, "Proxy Authen Challenge"},
    {32, 0, &ignore_avp6, "Proxy Authen ID"},
    {33, 1, &ignore_avp6, "Proxy Authen Response"},
    {34, 1, NULL, "Call Errors"},
    {35, 1, &ignore_avp6, "ACCM"},
    {36, 1, &rand_vector_avp6, "Random Vector"},
    {37, 1, NULL, "Private Group ID"},
    {38, 0, &rx_speed_avp6, "Receive Connect Speed"},
    {39, 1, &seq_reqd_avp6, "Sequencing Required"}
};

void wrong_length6 (struct call6 *c, char *field, int expected, int found,
                   int min)
{
    if (min)
...
int message_type_avp6 (struct tunnel6 *t, struct call6 *c, void *data,
                      int datalen)
{
    /*
    ...
    if (t->sanity)
    {
        ...
        if ((c != t->self) && (c->msgtype < Hello))
        {
            if (DEBUG)
                l2tp_log (LOG_DEBUG,
                     "%s: attempting to negotiate tunnel6 inside a call6!\n",
                     __FUNCTION__);
            return -EINVAL;
        }
        ...
        case ICRQ:
            ...
            if (c != t->self)
            {
                if (DEBUG)
                    l2tp_log (LOG_DEBUG,
                         "%s: attempting to negotiate ICRQ on a call6!\n",
                         __FUNCTION__);
                return -EINVAL;
            }
            break;
        case ICRP:
            if (t->state != SCCCN)
            {
                if (DEBUG)
                    l2tp_log (LOG_DEBUG,
                         "%s: attempting to negotiate ICRP on tunnel6!=SCCCN\n",
                         __FUNCTION__);
                return -EINVAL;
            }
            if (c->state != ICRQ)
        ...
        case OCRP:             /* jz: case for ORCP */
            if (t->state != SCCCN)
            {
                if (DEBUG)
                    l2tp_log (LOG_DEBUG,
                         "%s: attempting to negotiate OCRP on tunnel6!=SCCCN\n",
                         __FUNCTION__);
                return -EINVAL;
            }
            if (c->state != OCRQ)
    ...
    if (c->msgtype == ICRQ)
    {
        ...
        {
            if (DEBUG)
                l2tp_log (LOG_DEBUG, "%s: new incoming call6\n", __FUNCTION__);
        }
        tmp = new_call6 (t);
        if (!tmp)
        {
            l2tp_log (LOG_WARNING, "%s: unable to create new call6\n", __FUNCTION__);
            return -EINVAL;
        }
...
int rand_vector_avp6 (struct tunnel6 *t, struct call6 *c, void *data,
                     int datalen)
{
    int size;
    struct unaligned_u16 *raw = data;
...
int ignore_avp6 (struct tunnel6 *t, struct call6 *c, void *data, int datalen)
{
    /*
...
int seq_reqd_avp6 (struct tunnel6 *t, struct call6 *c, void *data, int datalen)
{
#ifdef SANITY
    if (t->sanity)
    {
...
int result_code_avp6 (struct tunnel6 *t, struct call6 *c, void *data,
                     int datalen)
{
    /*
...
int protocol_version_avp6 (struct tunnel6 *t, struct call6 *c, void *data,
                          int datalen)
{
    /*
     * Find out what version of l2tp the other side is using.
...
int framing_caps_avp6 (struct tunnel6 *t, struct call6 *c, void *data,
                      int datalen)
{
    /*
...
int bearer_caps_avp6 (struct tunnel6 *t, struct call6 *c, void *data,
                     int datalen)
{
    /*
...
int firmware_rev_avp6 (struct tunnel6 *t, struct call6 *c, void *data,
                      int datalen)
{
    /*
...
int bearer_type_avp6 (struct tunnel6 *t, struct call6 *c, void *data,
                     int datalen)
{
    /*
...
int frame_type_avp6 (struct tunnel6 *t, struct call6 *c, void *data, int datalen)
{
    /*
...
int hostname_avp6 (struct tunnel6 *t, struct call6 *c, void *data, int datalen)
{
    /*
...
int dialing_number_avp6 (struct tunnel6 *t, struct call6 *c, void *data,
                        int datalen)
{
    /*
     * What is the peer's name?
...
int dialed_number_avp6 (struct tunnel6 *t, struct call6 *c, void *data,
                       int datalen)
{
    /*
     * What is the peer's name?
...
int sub_address_avp6 (struct tunnel6 *t, struct call6 *c, void *data,
                     int datalen)
{
    /*
     * What is the peer's name?
...
int vendor_avp6 (struct tunnel6 *t, struct call6 *c, void *data, int datalen)
{
    /*
...
int challenge_avp6 (struct tunnel6 *t, struct call6 *c, void *data, int datalen)
{
    /*
...
int chalresp_avp6 (struct tunnel6 *t, struct call6 *c, void *data, int datalen)
{
    /*
     * We are sent a challenge
     */
...
int assigned_tunnel_avp6 (struct tunnel6 *t, struct call6 *c, void *data,
                         int datalen)
{
    /*
    ...
    if (t->sanity)
    {
        ...
        default:
            if (DEBUG)
                l2tp_log (LOG_DEBUG,
                     "%s: tunnel6 ID not appropriate for message %s.  Ignoring.\n",
                     __FUNCTION__, msgtypes[c->msgtype]);
            return 0;
        }
        if (datalen != 8)
        {
            if (DEBUG)
                l2tp_log (LOG_DEBUG,
                     ...
                     datalen);
            wrong_length (c, "Assigned tunnel6 ID", 8, datalen, 0);
            return -EINVAL;
        }
    }
#endif
    ...
    if (gconfig.debug_avp)
    {
        if (DEBUG)
            l2tp_log (LOG_DEBUG,
                 "%s: using peer's tunnel6 %d\n", __FUNCTION__,
                 ntohs (raw[3].s));
    }
    return 0;
}

int assigned_call_avp6 (struct tunnel6 *t, struct call6 *c, void *data,
                       int datalen)
{
    /*
    ...
    if (t->sanity)
    {
        ...
        default:
            if (DEBUG)
                l2tp_log (LOG_DEBUG,
                     "%s: call6 ID not appropriate for message %s.  Ignoring.\n",
                     __FUNCTION__, msgtypes[c->msgtype]);
            return 0;
        }
        if (datalen != 8)
        {
            if (DEBUG)
                l2tp_log (LOG_DEBUG,
                     "%s: avp is wrong size.  %d != 8\n", __FUNCTION__,
                     datalen);
            wrong_length (c, "Assigned call6 ID", 8, datalen, 0);
            return -EINVAL;
        }
    }
#endif
    ...
    if (gconfig.debug_avp)
    {
        if (DEBUG)
            l2tp_log (LOG_DEBUG,
                 "%s: using peer's call6 %d\n", __FUNCTION__, ntohs (raw[3].s));
    }
    return 0;
}

int packet_delay_avp6 (struct tunnel6 *t, struct call6 *c, void *data,
                      int datalen)
{
    /*
     * What is their CID that we must use from now on?
...
int call_serno_avp6 (struct tunnel6 *t, struct call6 *c, void *data, int datalen)
{
    /*
...
int rx_speed_avp6 (struct tunnel6 *t, struct call6 *c, void *data, int datalen)
{
    /*
...
int tx_speed_avp6 (struct tunnel6 *t, struct call6 *c, void *data, int datalen)
{
    /*
...
int call_physchan_avp6 (struct tunnel6 *t, struct call6 *c, void *data,
                       int datalen)
{
    /*
...
int receive_window_size_avp6 (struct tunnel6 *t, struct call6 *c, void *data,
                             int datalen)
{
    /*
     * What is their RWS?
...
int handle_avps6 (struct buffer6 *buf, struct tunnel6 *t, struct call6 *c)
{
    /*
    ...
    if (gconfig.debug_avp)
        l2tp_log (LOG_DEBUG, "%s: handling avp's for tunnel6 %d, call %d\n",
    ...
    while (len > 0)
    {
        ...
        if (ALENGTH (avp->length) > len)
        {
            l2tp_log (LOG_WARNING,
                 ...
                 __FUNCTION__);
            set_error6 (c, ERROR_LENGTH, "Invalid AVP length");
            c->needclose = -1;
            return -EINVAL;
        }
        if (avp->attr && firstavp)
        {
            l2tp_log (LOG_WARNING, "%s: First AVP was not message type.\n",
                 __FUNCTION__);
            set_error6 (c, VENDOR_ERROR, "First AVP must be message type");
            c->needclose = -1;
            return -EINVAL;
        }
        if (ALENGTH (avp->length) < sizeof (struct avp_hdr))
        {
            ...
            set_error6 (c, ERROR_LENGTH, "AVP too small");
            c->needclose = -1;
            return -EINVAL;
        }
        if (AZBITS (avp->length))
        {
            ...
            if (AMBIT (avp->length))
            {
                set_error6 (c, ERROR_RESERVED, "reserved bits set in AVP");
                c->needclose = -1;
                return -EINVAL;
            }
            goto next;
        ...
        if (AHBIT (avp->length))
        {
            ...
            if (decrypt_avp6 (data, t))
            {
                ...
                if (AMBIT (avp->length))
                {
                    set_error6 (c, VENDOR_ERROR, "Invalid Hidden AVP");
                    c->needclose = -1;
                    return -EINVAL;
                }
                goto next;
            ...
            hidlen = 0;
        if (avps6[avp->attr].handler)
        {
            if (avps6[avp->attr].handler (t, c, avp, ALENGTH (avp->length)))
            {
                if (AMBIT (avp->length))
                {
                    l2tp_log (LOG_WARNING,
        ...
        else
        {
            if (AMBIT (avp->length))
            {
                l2tp_log (LOG_WARNING,
                     ...
                     __FUNCTION__, avp->attr, avps6[avp->attr].description,
                     (c != t->self) ? "call6" : "tunnel6");
                set_error6 (c, VENDOR_ERROR, "No handler for attr %d (%s)\n",
                           avp->attr, avps6[avp->attr].description);
                return -EINVAL;
            }
            else
            {
