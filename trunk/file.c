...
int init_config ()
{
    ...
    gconfig.ipaddr.listenaddr = htonl(INADDR_ANY); /* Default is to bind (listen) to all interfaces */
    gconfig.debug_avp = 0;
...
//    myaddr.ai_family = PF_INET6;	//TODO: need to see
...
//    ipr->next = NULL;
//    //RY: start
// //   getaddrinfo(value, NULL, AF_INET6, &myaddr, &hp);
//    //hp = gethostbyname (value);
//    //RY: end here
//    //if (!hp)
//    {
...
//        return NULL;
//    }
//    //bcopy (hp->ai_addr, &ipr->start, sizeof (unsigned int));
//    if (c)
//    {
//		e = d;
//		while(*e != '\0') {
...
int set_listenaddr (char *word, char *value, int context, void *item)
{
    ...
    case CONTEXT_GLOBAL:
        ...
        l2tp_log (LOG_DEBUG, "set_listenaddr: Setting listen address to %s\n",
             value);
#endif
        if (set_ip (word, value, &(((struct global *) item)->ipaddr.listenaddr)))
		return -1;
	break;
    default:
        snprintf (filerr, sizeof (filerr), "'%s' not valid in this context\n",
...
//                     s);
                ...
                printf("RY:parse_config: line %d: unknown context '%s'\n", linenum,
                     s);
                //RY: end
                return -1;
            }
