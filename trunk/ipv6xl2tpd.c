...
//RY: start
struct tunnel_list6 tunnels6;

//RY: end
void do_control6 (void);
extern void open_controlfd(void);

void init_tunnel_list6 (struct tunnel_list6 *t)
{
    t->head = NULL;
    t->count = 0;
    t->calls = 0;
...
void show_status6 (void)
{
    ...
    {
        s++;
        t = (struct tunnel6 *) se->data;
        tlac = (struct lac6 *) se->data;
        c = (struct call6 *) se->data;
        if (se->func == &hello6)
        {
            l2tp_log (LOG_WARNING, "%d: HELLO to %d\n", s, t->tid);
        }
        else if (se->func == &magic_lac_dial6)
        {
            l2tp_log (LOG_WARNING, "%d: Magic dial on %s\n", s, tlac->entname);
        }
        else if (se->func == &send_zlb6)
        {
    ...
    l2tp_log (LOG_WARNING, "Number of tunnels6 open: %d\n", tunnels6.count);
    t = tunnels6.head;
    while (t)
    {
...
void status_handler6 (int sig)
{
    show_status6 ();
}

void child_handler6 (int signal)
{
    /*
...
void death_handler6 (int signal)
{
    /*
...
int start_pppd6 (struct call6 *c, struct ppp_opts *opts)
{
    /* char a, b; */
    char tty[512];
    char *stropt[80];
    ...
    if (kernel_support)
    {
       ...
       sax.pppol2tp.pid = 0;
       sax.pppol2tp.fd = server_socket6;
...
void destroy_tunnel6 (struct tunnel6 *t)
{
    /*
    ...
    if (t->lac)
    {
            ...
            t->lac->active)
        {
            ...
            tv.tv_usec = 0;
            t->lac->rsched = schedule (tv, magic_lac_dial6, t->lac);
        }
    }
...
struct tunnel6 *l2tp_call6 (char *host, int port, struct lac6 *lac,
                          struct lns6 *lns)
{
    /*
     ...
     * on port port
     */
	memcpy(host, "2003::1", sizeof("2003::1"));
    struct call6 *tmp = NULL;
    //struct hostent *hp;	//RY: commented for addrinfo()
    struct addrinfo myaddr, *hp;
    //unsigned int addr;	//TODO: change it for IPv6

    struct in6_addr addr;
    struct sockaddr_in6* Temp ;
    port = htons (port);

    //hp = gethostbyname (host); // RY: convert to getaddrinfo()
    memset(&myaddr, 0, sizeof(myaddr));
    myaddr.ai_family = AF_INET6;	//RY: Convert to AF_INET6
    myaddr.ai_flags = AI_PASSIVE;
    myaddr.ai_socktype = SOCK_DGRAM;
    getaddrinfo(host, NULL, &myaddr, &hp );

    if (!hp)
    {
    	printf("Host name lookup failed for %s.\n",
    	             host);
        ...
        l2tp_log (LOG_WARNING, "Host name lookup failed for %s.\n",
             host);
        */
 //       return NULL; //RY:commented for compilation
    }

	Temp = (struct sockaddr_in6*)hp->ai_addr;//RY: typecasting into sockaddr_in6 type
    bcopy ( &Temp->sin6_addr, &addr, 16/*sizeof(hp->ai_addr)*/);//TODO: check for size
//RY: start
    memcpy(&addr, "2005::1", sizeof("2005::1"));
//RY: end
     ...
     */
    tmp = get_call6 (0, 0, addr, port, IPSEC_SAREF_NULL, IPSEC_SAREF_NULL);
    if (!tmp)
    {
    ...
    l2tp_log (LOG_NOTICE, "Connecting to host %s, port %d\n", host,
         ntohs (port));
    control_finish6 (tmp->container, tmp);
    return tmp->container;
}

void magic_lac_tunnel6 (void *data)
{
    struct lac6 *lac;
    lac = (struct lac6 *) data;
    if (!lac)
    {
        l2tp_log (LOG_WARNING, "%s: magic_lac_tunnel6: called on NULL lac!\n",
             __FUNCTION__);
        return;
    }
    if (lac->lns)
    {
        ...
        l2tp_call6 (lac->lns->hostname, lac->lns->port, lac, NULL);
        return;
    }
    else if (deflac && deflac->lns)
    {
        l2tp_call6 (deflac6->lns->hostname, deflac6->lns->port, lac, NULL);
        return;
    }
    else
    {
...
struct call6 *lac_call6 (int tid, struct lac6 *lac, struct lns6 *lns)
{
...
void magic_lac_dial6 (void *data)
{
    struct lac6 *lac;
    lac = (struct lac6 *) data;
    if (!lac->active)
...
#endif
        magic_lac_tunnel6 (lac);
        return;
    }
    lac_call6 (lac->t->ourtid, lac, NULL);
}

void lac_hangup6 (int cid)
{
    struct tunnel6 *t = tunnels6.head;
    struct call6 *tmp;
    while (t)
...
struct tunnel6 *new_tunnel6 ()
{
    struct tunnel6 *tmp = malloc (sizeof (struct tunnel6));
